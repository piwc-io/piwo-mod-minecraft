    ...
    implements IWorldAccess
{
    ...
    public void renderEntities(Vec3D vec3d, ICamera icamera, float f)
    {
        ...
        TileEntityRenderer.instance.func_22267_a(worldObj, renderEngine, mc.fontRenderer, mc.renderViewEntity, f);
        RenderManager.instance.func_22187_a(worldObj, renderEngine, mc.fontRenderer, mc.renderViewEntity, mc.gameSettings, f);
        field_1423_J = 0;
        field_1422_K = 0;
        field_1421_L = 0;
        EntityLiving entityliving = mc.renderViewEntity;
        ...
        for(int i = 0; i < list.size(); i++)
        {
            ...
            if(entity.isInRangeToRenderVec3D(vec3d) && icamera.isBoundingBoxInFrustum(entity.boundingBox) && (entity != mc.renderViewEntity || mc.gameSettings.thirdPersonView || mc.renderViewEntity.isPlayerSleeping()) && worldObj.blockExists(MathHelper.floor_double(entity.posX), MathHelper.floor_double(entity.posY), MathHelper.floor_double(entity.posZ)))
            {
                field_1422_K++;
    ...
    public void renderSky(float f)
    {
        ...
        tessellator.draw();
        GL11.glDisable(3553 /*GL_TEXTURE_2D*/);
        float f13 = worldObj.getStarBrightness(f);
        if(f13 > 0.0F)
        {
    ...
    public void renderClouds(float f)
    {
        ...
        double d = mc.renderViewEntity.prevPosX + (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * (double)f + (double)(((float)field_1435_x + f) * 0.03F);
    ...
    public void renderCloudsFancy(float f)
    {
        ...
        float f2 = 12F;
        float f3 = 4F;
        double d = (mc.renderViewEntity.prevPosX + (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * (double)f + (double)(((float)field_1435_x + f) * 0.03F)) / (double)f2;
